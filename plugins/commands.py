...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        buttons = [[ InlineKeyboardButton(f' 📢update Channel💬 ', url='https://t.me/+sse5wRfFF3FmY2I9') ]] 
        m=await message.reply_text(text=f"<i>ഈ ബോട്ട് പ്രവർത്തിക്കുന്നത് ഫിലിം ഫാക്ടറി എന്ന ഗ്രൂപ്പിന് വേണ്ടി ആണ്. അതിനാൽ എന്നെ മറ്റൊരിടത്തും ഉപയോഗിക്കാൻ കഴിയില്ല.താഴെ കാണുന്ന ലിങ്കിൽ ക്ലിക്ക് ചെയ്ത് ഗ്രൂപ്പിൽ ജോയിൻ ചെയ്താൽ നിങ്ങൾക്ക് മൂവീസ് റിക്വസ്റ്റ് ചെയ്യാം. OTT ഇറങ്ങിയ എല്ലാ പുതിയ മൂവിസും ലഭ്യമാണ്, പ്രിന്റ് ഫയൽസ് കിട്ടില്ല. നിങ്ങളുടെ ഫ്രണ്ട്‌സ്നെ കൂടി നമ്മുടെ ഗ്രൂപ്പിൽ ആഡ് ചെയ്യാൻ മറക്കല്ലേ..\n💖JOIN GROUP - https://t.me/+rjw2I6MtjW8xYzRl</i>", reply_markup=InlineKeyboardMarkup(buttons))         
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        await client.send_message(
            ...
            text="**⚠️നോട്ടീസ്⚠️\n\n അതെ ബ്രോ താഴെ കാണുന്ന ബട്ടൺ ക്ലിക്ക് ചെയ്ത് ചാനലിൽ ജോയിൻ ചെയ്ത ശേഷം 💖[TRY AGAIN]💖 കൊടുത്താൽ നിങ്ങൾക്ക് ഫയൽ കിട്ടും 🤷🏻‍♂️.**",
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        try:
            ...
            buttons = [[
                InlineKeyboardButton('Ok,Get File📩', url=f"https://telegram.dog/{temp.U_NAME}?start=files_{file_id}#{message.from_user.id}")
            ]]
            await xd.edit(          
                text=f"<b>-ғɪʟᴇ ᴅᴇᴛᴀɪʟs-</b>\n\n📁ғɪʟᴇ ɴᴀᴍᴇ - <code>{name}</code>\n🪫ғɪʟᴇ sɪᴢᴇ - <code>{size}<code/>\n\n•ഫയൽ നെയിം വായിച്ചു നോക്കിയ ശേഷം മാത്രം get file ക്ലിക്ക് ചെയ്യുക.\n\n•❗️Error- ചിലപ്പോൾ file കിട്ടിയാൽ ഉടൻ തന്നെ file delete ആയിപോകാം, കൂടെ ഈ മെസ്സജ് ഡിലീറ്റ് ആയാൽ ഗ്രൂപ്പിൽപോയി ഒന്നുംകൂടി ട്രൈ ചെയ്യുക, ഈ മെസ്സജ് ഡിലീറ്റ് ആയില്ല എങ്കിൽ get file ബട്ടൻ ക്ലിക്ക് ചെയ്താൽ മതി, പിന്നെ file ഫോർവേഡ് ചെയ്ത ശേഷം ഡൌൺലോഡ് ചെയ്യുക.",
                reply_markup=InlineKeyboardMarkup(buttons)
            ) 
        except Exception as e:
            logger.exception(e)

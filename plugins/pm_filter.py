            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"𓆏{get_size(file.file_size)}﹅ {file.file_name}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=XD-{file.file_id}")
                    ),
                ]
                for file in files
            ]
    ...
    if ENABLE_SHORTLINK == True:
        if settings["button"]:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"𓆏{get_size(file.file_size)}﹅ {file.file_name}", url=await get_shortlink(message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=XD-{file.file_id}")
                    ),
                ]
                for file in files
            ]

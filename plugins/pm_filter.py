    ...
    if ENABLE_SHORTLINK == True:
        if settings['button']:
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{file.file_name}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=XD-{file.file_id}")
                    ),
                    InlineKeyboardButton(
        ...
        if settings['button']:
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{file.file_name}", url=f"https://telegram.dog/{temp.U_NAME}?start=XD-{file.file_id}"
                    ),
                    InlineKeyboardButton(
                        text=f"{get_size(file.file_size)}",
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                if clicked == typed:
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start=files_{ident}_{file_id}")
                    return
                else:
            ...
            await query.answer(url=f"https://t.me/{temp.U_NAME}?start=XD-{ident}_{file_id}")
        except Exception as e:
            await query.answer(url=f"https://t.me/{temp.U_NAME}?start=XD-{ident}_{file_id}")
        ...
        await query.answer("🪛നിങ്ങൾ അയച്ച വാക്കിൽ ഒരു ഫയൽ എന്റെ പക്കലില്ല, ചിലപ്പോൾ നിങ്ങൾ അയച്ച വാക്ക് തെറ്റായിരിക്കും.ഈ മൂവി OTT റിലീസ് ആയെങ്കിൽ സ്പെല്ലിങ് ചെക്ക് ചെയ്ത ശേഷം ഒന്നുംകൂടെ അയക്കുക.", show_alert=True)

    elif query.data == "reqinfo":
        await query.answer("⚠ ɪɴꜰᴏʀᴍᴀᴛɪᴏɴ ⚠\n\nᴀꜰᴛᴇʀ 10 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ\n\nɪꜰ ʏᴏᴜ ᴅᴏ ɴᴏᴛ ꜱᴇᴇ ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴏᴠɪᴇ / sᴇʀɪᴇs ꜰɪʟᴇ, ʟᴏᴏᴋ ᴀᴛ ᴛʜᴇ ɴᴇxᴛ ᴘᴀɢᴇ\n\n❣ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴍᴀʟᴀyᴀʟᴀᴍᴠɪʙᴇ", show_alert=True)

    elif query.data == "minfo":
        await query.answer("⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯\nᴍᴏᴠɪᴇ ʀᴇǫᴜᴇꜱᴛ ꜰᴏʀᴍᴀᴛ\n⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯⋯\n\nɢᴏ ᴛᴏ ɢᴏᴏɢʟᴇ ➠ ᴛʏᴘᴇ ᴍᴏᴠɪᴇ ɴᴀᴍᴇ ➠ ᴄᴏᴘʏ ᴄᴏʀʀᴇᴄᴛ ɴᴀᴍᴇ ➠ ᴘᴀꜱᴛᴇ ᴛʜɪꜱ ɢʀᴏᴜᴘ\n\nᴇxᴀᴍᴘʟᴇ : ᴀᴠᴀᴛᴀʀ: ᴛʜᴇ ᴡᴀʏ ᴏғ ᴡᴀᴛᴇʀ\n\n🚯 ᴅᴏɴᴛ ᴜꜱᴇ ➠ ':(!,./)\n\n©️ ᴍᴀʟᴀyᴀʟᴀᴍᴠɪʙᴇ", show_alert=True)
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            if not files:
                ...
                buttons = [[
                    InlineKeyboardButton('◀tips▶', callback_data='tinfo')
                    ],[
                ...
                z = await message.reply_photo(photo="https://telegra.ph/file/acb82cc515d027857a0f7.jpg", caption=f"<b>HI {message.from_user.mention} Read this Point's, \n•ചിലപ്പോൾ നിങ്ങൾ അയച്ച സ്പെല്ലിങ് തെറ്റ് ആകാം..\n•നിങ്ങൾ ചോദിച്ച മൂവി OTT oR DVD റിലീസ് ആയി കാണില്ല.\n•google നോക്കി ശെരിയായ സ്പെല്ലിങ് മാത്രം കോപ്പി ചെയ്ത് അയക്കുക.\n•നിങ്ങൾ ചോദിച്ച ഫയൽ എന്റെ ഡാറ്റബേസിൽ കാണില്ല.\n•ഇംഗ്ലീഷ് ലാംഗ്വേജ് മാത്രമേ എനിക്ക് മനസ്സിലാകൂ അതിനാൽ ഇംഗിഷ് മാത്രം ഉപയോഗിക്കുക.\n📍ഇതിൽ ഏതെങ്കിലും കാരണം കൊണ്ട് ആക്കാം നിങ്ങൾക്ക് ഫയൽ കിട്ടാത്തത്.\n➖️➖️➖️➖️➖️➖️➖️➖️➖️➖️\n•Sometimes the spelling you sent may be wrong..\n•The movie you asked for is not found as OTT oR DVD release.\n•Please check google and copy the correct spelling and send.\n•The file you asked for is not found in my database.\n•English language only I understand so please use English only.\n📍You may not get the file due to any of these reasons.</b>", reply_markup = InlineKeyboardMarkup(buttons))   
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                logger.exception(e)
                fek = await message.reply_video(video=NOR_IMG, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
                try:
                    if settings['auto_delete']:
                        await asyncio.sleep(600)
                        await fek.delete()
        ...
        if message.chat.id == SUPPORT_CHAT_ID:
            ...
            fuk = await message.reply_video(video=NOR_IMG, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(600)
                    await fuk.delete()

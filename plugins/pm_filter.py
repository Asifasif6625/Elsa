...
from info import ADMINS, AUTH_CHANNEL, FILE_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, NOR_IMG, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, SPELL_IMG, MSG_ALRT, MAIN_CHANNEL, LOG_CHANNEL, PICS, SUPPORT_CHAT_ID
    ...
    if ENABLE_SHORTLINK == True:
        ...
        if settings['button']:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"𓆏{get_size(file.file_size)}﹅ {file.file_name}", callback_data=f'files#{file.file_id}'
                    ),
                ]
                for file in files
            ]
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                             [
                              ...
                              InlineKeyboardButton("⚠️ 𝖢𝖺𝗇'𝗍 𝖠𝖼𝖼𝖾𝗌𝗌 ❓ 𝖢𝗅𝗂𝖼𝗄 𝖧𝖾𝗋𝖾 ⚠️", url='https://t.me/howtogtmv/72')
                             ]
                            ]
                        )
                    )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            if not files:
                buttons = [[types.InlineKeyboardButton('💬ᴍꜱɢ ɪɴ ᴍᴀʟᴀyᴀʟᴀᴍ💬', callback_data='makri')]]
                z = await message.reply_text(text=f"<i>{message.from_user.mention}🪛I don't have a file in the word you sent, sometimes the word you sent is wrong.If this movie is an OTT release please check the spelling and then send it.</i>", reply_markup=types.InlineKeyboardMarkup(buttons))             
